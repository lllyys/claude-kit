...
1) **Discover candidate servers**
   ...
   - Scan for MCP-related dependencies in this project's manifest (e.g. `package.json`, `pyproject.toml`, `go.mod`, `Cargo.toml` — whatever applies).
...
- For stdio servers launched via a package runner (e.g. `npx`, `uvx`), the package is fetched when the server first runs.

## Example
```json
// .mcp.json — a stdio server launched via a package runner
{
  "mcpServers": {
    "example-server": {
      "command": "npx",
      "args": ["-y", "@example-org/mcp-server@latest"]
    }
  }
}
```
