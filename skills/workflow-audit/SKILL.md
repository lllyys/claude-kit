...
| Toolchain setup action below its current major (e.g. a `setup-*` / `*/action-setup` for this project's language or package manager) | **high** | Upgrade to the latest major for that action. |
...
| Check | Severity | Rule |
|-------|----------|------|
| Different language/runtime version across workflows (e.g. `node-version`, `python-version`, `go-version`) | **high** | All workflows should pin the same toolchain version. |
| Different package-manager version across workflows | **high** | All workflows should use the same package-manager version. |
