...
- User reports a problem: "X doesn't work", "I found a bug", "the app hangs"
...
```
User: "feature X crashes when I give it an empty input"
→ Investigate: the feature X handler exists (e.g. the input-processing module), but it has no empty/null guard before the main code path
→ Classification: Bug (implemented but broken)
→ Record: bug #106 in docs/bugs.md
→ Report: "Triaged: bug #106 | Area: feature-X handler | Crashes on empty input — missing guard"
```

**Example 2: Feature request**

```
User: "can we add support for Y?"
→ Investigate: the related capability exists but has no support for Y
→ Classification: Feature (never implemented)
→ Record: feature #43 in docs/features.md
→ Report: "Triaged: feature #43 | Area: <module> | Support for Y not yet implemented"
```

**Example 3: Duplicate**

```
User: "the results panel doesn't render — I just see a blank area"
→ Investigate: results rendering exists, find existing bug #99
