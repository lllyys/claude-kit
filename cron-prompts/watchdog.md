...
3. To renew: `CronDelete` the existing job, then `CronCreate` with the prompt read from the corresponding file:
   - verify cron — `9 * * * *` — `${CLAUDE_PLUGIN_ROOT}/cron-prompts/verify.md`
   - bugfix cron — `39 * * * *` — `${CLAUDE_PLUGIN_ROOT}/cron-prompts/bugfix.md`
   - watchdog cron — `54 4 * * *` — `${CLAUDE_PLUGIN_ROOT}/cron-prompts/watchdog.md`
   - feature cron — `24 */2 * * *` — `${CLAUDE_PLUGIN_ROOT}/cron-prompts/feature.md`
...
`.claude/rules/49-background-shells.md` codifies the anti-patterns that produced the ghost-shell incident: `pgrep -f "<toolname>"` polling loops keyed on a class of work get re-armed by unrelated later invocations. The watchdog is the right place to sweep for these post-hoc.

5. Scan for orphan ghost-poll patterns — the shape of the anti-pattern (a poll/sleep loop), not any specific tool name:

   ```bash
   ps -eo pid,etime,command | grep -E "while .*pgrep|until .*pgrep|pgrep -f .*(test|build|dev|serve|watch)|until [^;]*sleep [0-9]+; *done|while [^;]*sleep [0-9]+; *done" | grep -v grep
   ```
...
7. Also scan for stale dev-server / test-runner / build processes that have been running > 30 min — these can also become orphaned across sessions (a never-torn-down dev server or a hung test run are the usual culprits). Match on generic dev/test/build runner shapes rather than naming this project's specific tools:

   ```bash
   ps -eo pid,etime,command | grep -E "(test|build|dev|serve|watch)( |$)|run .*(test|build|dev)" | grep -v grep
   ```
